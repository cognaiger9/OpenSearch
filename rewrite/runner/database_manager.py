...
class DatabaseManager:
    ...
    def __new__(cls, db_root_path=None, db_id=None):
        if (db_root_path is not None) and (db_id is not None):
            with cls._lock:
                if cls._instance is None:
                    cls._instance = super(DatabaseManager, cls).__new__(cls)
                    cls._instance._init(db_root_path,db_id)
    ...
    def _set_paths(self):
        ...
        self.db_path = Path(self.db_root_path)/ "database_en" / self.db_id / f"{self.db_id}.sqlite"
        self.db_directory_path = Path(self.db_root_path)/"database_en" / self.db_id
        self.db_json = Path(self.db_root_path)/"data_preprocess"/"dev.json"
